function ecg = perceive_ecg(data, fs, plotit)
    ...
    if ~isempty(i)
        ...
        for a = 1:length(i)
            try ndata2(a,:) = data(i(a) - round(.05*fs):i(a)+round(.1*fs));
            end
        end
        ...
        for a=1:length(h)
            ...
            if ~isempty(ix)
                dd = 60./ (diff(ix)/fs);
                thr2(a) = nansum(dd>55&dd<120)./std(dd);
            end
        end

        [~,ithr2] = max(thr2);
        ecg.proc.thresh2=h(ithr2);
        ...
        if plotit
            ...
            xlim([4 30]);legend('original','cleaned'); 
            xlabel('Frequency [Hz]');
            ylabel('Relative spectral power [%]');
            title([' HR: ' num2str(ecg.hr,3) '/min  N = ' num2str(ecg.stats.n,3)]);
            subplot(2,2,3:4);
            plot(t,data,'color','r');    
            hold on;
